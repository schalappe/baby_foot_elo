...
interface EntityStatsCardsProps {
  stats: EntityStats;
  entityType: 'player' | 'team'; // For minor copy differences if needed
}
 ...
 *     For copy or display differences.
 *
 * Returns
 * -------
 * JSX.Element
 ...
 */
const EntityStatsCards: React.FC<EntityStatsCardsProps> = ({ stats, entityType }) => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <Card className="bg-card text-card-foreground shadow-lg rounded-xl overflow-hidden relative">
        <CardContent className="flex flex-col items-center justify-center p-6 h-full text-left">
          ...
          <div className="text-4xl sm:text-5xl font-bold mt-2 mb-1">{stats.global_elo}</div>
        ...
        <CardContent className="pb-6 pt-0 px-2">
          {stats.elo_values && stats.elo_values.length > 1 ? (
            <ChartContainer config={{ elo: { label: 'ELO', color: 'var(--chart-1)' } }} className="w-full h-40">
              <LineChart data={[...stats.elo_values].reverse().map((elo: number, idx: number) => ({ match: idx + 1, elo }))} margin={{ top: 0, right: 16, left: 12, bottom: 0 }}>
          ...
          <div className="flex justify-between mt-4 px-2">
            <div className="flex flex-col items-center">
              <span className="text-lg font-bold" style={{ color: 'var(--win-text)' }}>{stats?.wins ?? 0}</span>
              <span className="text-xs text-muted-foreground">Victoires</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-lg font-bold" style={{ color: 'var(--lose-text)' }}>{stats?.losses ?? 0}</span>
              <span className="text-xs text-muted-foreground">Défaites</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-lg font-bold" style={{ color: stats ? (stats.win_rate >= 0.5 ? 'var(--win-text)' : 'var(--lose-text)') : 'var(--muted)' }}>
      ...
      <Card className="bg-card text-card-foreground shadow-lg rounded-xl overflow-hidden">
        <CardContent className="flex flex-col items-center justify-center p-6 h-full space-y-2 text-center">
          <div className="relative w-32 h-32 sm:w-36 sm:h-36">
            {(() => {
              const currentWinRate = stats.recent?.win_rate ?? stats.win_rate ?? 0;
              const chartConfig = {
                winSegment: {
                  label: 'Win Rate',
          ...
          <div>
            <span className="text-lg sm:text-xl font-bold" style={{ color: 'var(--win-text)' }}>{stats.recent?.wins ?? stats.wins ?? 0}W</span>
            <span className="text-lg sm:text-xl font-bold"> - </span>
            <span className="text-lg sm:text-xl font-bold" style={{ color: 'var(--lose-text)' }}>{stats.recent?.losses ?? stats.losses ?? 0}L</span>
          </div>
          <div className="text-xs text-muted-foreground">{stats.recent?.matches_played ?? stats.matches_played ?? 0} dernières parties</div>
        </CardContent>
      </Card>
    </div>
  );
