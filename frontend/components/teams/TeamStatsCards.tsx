...
/**
 * Displays team statistics in cards with charts and badges.
 *
 * Parameters
 * ----------
 * stats : TeamStatistics
 *     The statistics object for the team.
 *
 * Returns
 * -------
 * JSX.Element
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <Card className="bg-card text-card-foreground shadow-lg rounded-xl overflow-hidden relative">
        <CardContent className="flex flex-col items-center justify-center p-6 h-full text-left">
          ...
          {stats.recent && stats.recent.elo_changes && stats.recent.elo_changes.length > 0 ? (() => {
          ...
          <CardContent className="pb-6 pt-0 px-2">
            ...
            {stats?.elo_values && stats.elo_values.length > 1 ? (
              <ChartContainer config={{ elo: { label: 'ELO', color: 'var(--chart-1)' } }} className="w-full h-40">
                <LineChart data={[...stats.elo_values].reverse().map((elo, idx) => ({ match: idx + 1, elo }))} margin={{ top: 0, right: 16, left: 12, bottom: 0 }}>
            ...
            <div className="flex justify-between mt-4 px-2">
              ...
              <div className="flex flex-col items-center">
                <span
                  className="text-lg font-bold" style={{ color: stats ? (stats.win_rate >= 0.5 ? 'var(--win-text)' : 'var(--lose-text)') : 'var(--muted)' }}
                >
                  {stats ? `${Math.round(stats.win_rate*100)}%` : '--'}
                </span>
      ...
      <Card className="bg-card text-card-foreground shadow-lg rounded-xl overflow-hidden">
        <CardContent className="flex flex-col items-center justify-center p-6 h-full space-y-2 text-center">
          <div className="relative w-32 h-32 sm:w-36 sm:h-36"> 
            {(() => {
              const currentWinRate = stats.recent?.win_rate ?? 0;
              const chartConfig = {
                winSegment: {
                  label: 'Win Rate',
          ...
          <div>
            <span className="text-lg sm:text-xl font-bold" style={{ color: 'var(--win-text)' }}>{stats.recent?.wins ?? 0}W</span>
            <span className="text-lg sm:text-xl font-bold"> - </span>
            <span className="text-lg sm:text-xl font-bold" style={{ color: 'var(--lose-text)' }}>{stats.recent?.losses ?? 0}L</span>
          </div>
          <div className="text-xs text-muted-foreground">{stats.recent?.matches_played ?? 0} dernières parties</div>
        </CardContent>
      </Card>
    </div>
  );
