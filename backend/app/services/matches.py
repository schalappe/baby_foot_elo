...
from app.db.repositories.players import batch_update_players_elo
from app.db.repositories.players_elo_history import (
    batch_record_player_elo_updates,
    get_player_elo_history_by_match_id,
)
from app.db.repositories.teams import batch_update_teams_elo
...
def get_matches(
    ...
    try:
        if team_id:
            matches_data = get_matches_by_team_id(
                team_id,
                limit=limit,
                offset=skip,
                is_fanny=is_fanny,
...
def delete_match(match_id: int) -> bool:
    ...
    try:
        ...
        success = delete_match_by_id(match_id)
        if not success:
...
def create_new_match(match_data: MatchCreate) -> MatchWithEloResponse:
    ...
    try:
        ...
        # ##: Create match record.
        match_id = create_match_by_team_ids(
            winner_team_id=match_data.winner_team_id,
            loser_team_id=match_data.loser_team_id,
            played_at=match_data.played_at,
            is_fanny=match_data.is_fanny,
...
def get_match_with_elo_player(match_id: int) -> MatchWithEloResponse:
    ...
    try:
        ...
        elo_history = get_player_elo_history_by_match_id(match_id)
...
def get_match_with_elo_team(match_id: int) -> MatchWithEloResponse:
    ...
    try:
        ...
        # ##: Get ELO history for this match.
        elo_history = get_team_elo_history_by_match_id(match_id)
